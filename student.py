...
class Policy(nn.Module):
    ...
    def critic_forward(self, x):
        # Same as actor_forward but for critic
        if x.dim() == 3:
            x = x.unsqueeze(0)
        if x.shape[-1] == 3:
            x = x.permute(0, 3, 1, 2)
    ...
    def _get_policy_params(self):
        # Get all parameters for the policy (actor + head)
    ...
    def train(self):
        ...
        for iteration in range(num_iterations):
            # Collect data
            states, actions, rewards, dones, values, log_probs, old_probs = [], [], [], [], [], [], []
            state, _ = env.reset()
            episode_reward = 0
            episode_rewards = []
    ...
    def _trpo_update(self, states, actions, advantages, old_log_probs, old_probs):
        ...
        actor_features = self.actor_forward(states)
        logits = self.policy_head(actor_features)
        probs = F.softmax(logits, dim=-1)
        ...
        old_params = torch.nn.utils.parameters_to_vector(policy_params).detach().clone()
    ...
    def _line_search(self, actor_features, actions, advantages, old_log_probs,
        ...
        torch.nn.utils.vector_to_parameters(old_params, params)
        for step_frac in [0.5 ** i for i in range(max_backtracks)]:
            new_params = old_params + step_frac * full_step
            torch.nn.utils.vector_to_parameters(new_params, params)
            with torch.no_grad():
                logits = self.policy_head(actor_features)
